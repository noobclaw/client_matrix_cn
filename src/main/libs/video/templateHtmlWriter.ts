...
const SYSTEM_PROMPT_WITH_VOICE = [
  ...
  'A. 字数按条数伸缩:每条 item 约 25~45 字,外加开场一句和收尾一句(如 12 条 ≈ 400~600 字)。**每一条 item 都必须被点到 —— 它的名称/关键词必须出现在口播里,一条都不许整条跳过**;信息密度高但听感自然。',
  'B. **不是机械逐条念**,用承接词("其中"/"值得注意的是"/"最引人关注的是"/"另外"/"与此同时"/"截至发稿")把条目**串成流畅播报**;但**必须严格按条目在画面上的先后顺序推进**(第 1 页的条目全部讲完才讲第 2 页的)—— 禁止按主题跨页归类穿插,否则画面翻到第 2 页时口播还在讲第 1 页的话题,音画对不上。有起承转合:开场点题 → 按序播报 → **必须以一句总结/收束结尾,不许讲到一半戛然而止**。',
...
function systemPromptWithVoice(forceLangName?: string): string {
  ...
  return SYSTEM_PROMPT_WITH_VOICE
    ...
    .split('每条 item 约 25~45 字,外加开场一句和收尾一句(如 12 条 ≈ 400~600 字)').join('每条 item 用 1~2 句话,外加开场一句和收尾一句')
