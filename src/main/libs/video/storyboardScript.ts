...
/**
 * 【纯本地】把用户写好的脚本格式化成分镜 —— 不调 LLM、不改一个字。
 *
 * 用户拍板(2026-08-04):「有详细脚本就不要再过一次 LLM,基本格式化一下直接给 Seedance」。
 * 所以这条路只做三件事:认结构标记 → 切镜 → 台词逐字照抄。
 *
 * 认得的写法(每镜之间空行或结构标记分隔):
 *   镜头1 / 分镜1 / 场景1 / Shot 1 / Scene 1   → 起新的一镜(其后的标题文字进 title)
 *   画面: / 视觉: / 场景描述:                   → visualFirst(只进图像模型,不会被念出来)
 *   旁白: / 台词: / 口播: / 配音:               → narration(逐字,拿去让 Seedance 念)
 *   运镜: / 镜头运动:                           → motion
 *   花字: / 字幕:                               → onScreenText
 *   音效: / 配乐:                               → sfx / bgmMood
 * 全角半角冒号都认。没有任何标记的普通段落 = 一段一镜,整段当台词。
 *
 * 时长:按语言的字/秒估(中日韩 4.5、其余 2.2),夹在 3~10 秒 —— Seedance 单镜的合理区间。
 */
export function formatScriptToShots(
  rawScript: string,
  lang: ContentLang,
): { shots: StoryShot[]; warnings: string[] } {
  const warnings: string[] = [];
  const src = String(rawScript || '').trim();
  if (!src) return { shots: [], warnings };
  const cps = charsPerSecond(lang);
  // ⚠️ 这里必须写 '\\s' / '\\d' —— 模板串里 `\s` 会被 JS 当【未知转义】把反斜杠丢掉,
  //    变成普通字母 s / d(实测:「镜头1」的数字就此匹配不上,整条结构识别失效)。
  const C = '[：:]';
  const reShotHead = new RegExp(`^\\s*(?:镜头|分镜|场景|shot|scene)\\s*([\\d一二三四五六七八九十]+)\\s*${C}?\\s*(.*)$`, 'i');
  const field = (names: string) => new RegExp(`^\\s*(?:${names})\\s*${C}\\s*(.*)$`, 'i');
  const reVisual = field('画面|视觉|场景描述|visual');
  const reNarr = field('旁白|台词|口播|配音|解说|narration|voice ?over|vo');
  const reMotion = field('运镜|镜头运动|运动|motion|camera');
  const reText = field('花字|字幕|大字|on ?screen ?text');
  ...
  const flush = () => {
    if (!cur) return;
    const hasContent = (cur.narration || '').trim() || (cur.visualFirst || '').trim();
    if (hasContent) shots.push(cur);
    cur = null;
  };

  for (const rawLine of src.split(/\r?\n/)) {
    const line = rawLine.trim();
    if (!line) { if (!sawMarker) flush(); continue; }   // 无标记脚本:空行分镜
    const head = reShotHead.exec(line);
    if (head) {
      sawMarker = true; flush(); cur = blank();
      if (head[2] && head[2].trim()) cur.title = head[2].trim();
      continue;
    }
    if (!cur) cur = blank();
    let m: RegExpExecArray | null;
    if ((m = reVisual.exec(line))) { sawMarker = true; cur.visualFirst = [cur.visualFirst, m[1].trim()].filter(Boolean).join(' '); cur.locked.push('visualFirst'); continue; }
    if ((m = reNarr.exec(line))) { sawMarker = true; cur.narration = [cur.narration, m[1].trim()].filter(Boolean).join(' '); cur.locked.push('narration'); continue; }
    if ((m = reMotion.exec(line))) { sawMarker = true; cur.motion = m[1].trim(); cur.locked.push('motion'); continue; }
    if ((m = reText.exec(line))) { sawMarker = true; cur.onScreenText = m[1].trim(); cur.locked.push('onScreenText'); continue; }
    if ((m = reSfx.exec(line))) { sawMarker = true; cur.sfx = m[1].trim(); cur.locked.push('sfx'); continue; }
    if ((m = reBgm.exec(line))) { sawMarker = true; cur.bgmMood = m[1].trim(); cur.locked.push('bgmMood'); continue; }
    // 普通正文行 = 台词(逐字)
    cur.narration = [cur.narration, line].filter(Boolean).join(' ');
    if (!cur.locked.includes('narration')) cur.locked.push('narration');
  }
  flush();

  // 无结构、且整篇挤成一段 → 按句切,避免一镜几百字(Seedance 单镜念不完)。
  if (shots.length === 1 && !sawMarker) {
    const one = shots[0];
    const sents = one.narration.split(/(?<=[。！？!?])\s*/).map((x) => x.trim()).filter(Boolean);
    if (sents.length > 1) {
      shots.length = 0;
      for (const s of sents) shots.push({ ...blank(), narration: s, locked: ['narration'] });
    }
  }

  for (const sh of shots) {
    const n = (sh.narration || '').trim();
    // 画面描述留空时用台词兜底:Seedance 至少知道这一镜在讲什么(不写死画面,让它自由发挥)。
    if (!sh.visualFirst) sh.visualFirst = n || sh.title || '';
    sh.seconds = Math.max(3, Math.min(10, Math.round((n.length / cps) * 10) / 10 || 5));
  }
  if (shots.length === 0) warnings.push('脚本里没解析出可用内容');
