...
const IDENTITY_EXPR: Record<string, string> = {
  ...
  x: '(function(){try{var avc=document.querySelector(\'[data-testid^="UserAvatar-Container-"]\');var handle=null,avatar=null,nickname=null;if(avc){handle=((avc.getAttribute("data-testid")||"").match(/UserAvatar-Container-(.+)/)||[])[1]||null;var img=avc.querySelector("img");if(img){avatar=img.src||null;nickname=img.getAttribute("alt")||null;}}var uid=(document.cookie.match(/twid=u%3D(\\d+)/)||[])[1]||null;return JSON.stringify({nickname:nickname,displayId:handle?("@"+handle):null,uid:uid,avatar:avatar});}catch(e){return "{}";}})()',
