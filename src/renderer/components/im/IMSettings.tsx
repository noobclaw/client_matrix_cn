...
// Platform metadata
const getPlatformMeta = (): Record<IMPlatform, { label: string; logo: string }> => {
  return {
    dingtalk: { label: i18nService.t('imDingtalk'), logo: 'dingding.png' },
    feishu: { label: i18nService.t('imFeishuLark'), logo: 'feishu.png' },
    qq: { label: 'QQ', logo: 'qq_bot.jpeg' },
    telegram: { label: 'Telegram', logo: 'telegram.svg' },
    discord: { label: 'Discord', logo: 'discord.svg' },
    wecom: { label: i18nService.t('imWecom'), logo: 'wecom.png' },
  };
};
...
const IMSettings: React.FC = () => {
  ...
  const [language, setLanguage] = useState<string>(i18nService.getLanguage());
  ...
  const handleFeishuChange = (field: 'appId' | 'appSecret' | 'domain', value: string) => {
      ...
      <div className="flex-1 min-w-0 pl-4 pr-2 space-y-4 overflow-y-auto [scrollbar-gutter:stable]">
        ...
        <div className="flex items-center gap-3 pb-3 border-b dark:border-claude-darkBorder/60 border-claude-border/60">
          <div className="flex items-center gap-2">
            <div className="flex h-7 w-7 items-center justify-center rounded-md bg-white dark:bg-claude-darkBorder/30 p-1">
              <img
                src={getPlatformMeta()[activePlatform].logo}
                alt={getPlatformMeta()[activePlatform].label}
        ...
        {activePlatform === 'feishu' && (
          <div className="space-y-3">
            ...
            <div className="space-y-1.5">
              <label className="block text-xs font-medium dark:text-claude-darkTextSecondary text-claude-textSecondary">
                {i18nService.t('imFeishuPlatformLabel')}
              </label>
              <select
              ...
              <p className="text-xs dark:text-claude-darkTextSecondary text-claude-textSecondary opacity-60">
                {i18nService.t('imFeishuPlatformHint')}
              </p>
            </div>
