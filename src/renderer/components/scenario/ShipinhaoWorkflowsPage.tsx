/**
 * ShipinhaoWorkflowsPage — 视频号（WeChat Channels）平台工作流页面.
 *
 * 已挂 scenarios:
 *   shipinhao_image_text          — 用户填 3 段灵感,AI 改写 + 配图,发到视频号
 *   shipinhao_reply_fans_comment  — 在视频号助手逐条回复粉丝评论
 *
 * 结构照 DouyinWorkflowsPage,主色 green。视频号没有独立创作者子域,
 * channels.weixin.qq.com 本身就是创作后台,所以 requireCreatorCenter/creatorOnly 都为 false。
 */
...
interface Props {
  scenarios: Scenario[];           // already filtered to platform='shipinhao' by parent
  tasks: Task[];
  ...
  /** Jump to "My Tasks" filtered to 视频号 — used by 已达上限 modal CTA. */
  onGoToMyTasks?: () => void;
}

export const ShipinhaoWorkflowsPage: React.FC<Props> = ({
  scenarios,
  tasks,
  ...
  const FALLBACK_IMAGE_TEXT: Scenario = {
    ...
    version: '1.0.0',
    platform: 'shipinhao' as any,
    workflow_type: 'shipinhao_image_text_creation' as any,
    category: 'knowledge',
    name_zh: '视频号 · 图文创作',
    name_en: 'WeChat Channels Image-Text Creation',
    description_zh: '你填 3 段灵感来源，AI 改写成视频号图文动态。配图二选一：AI 生图 或 关键词抓网络图（2-6 张可调）。',
    description_en: 'Fill 3 source snippets; AI rewrites into a WeChat Channels image-text post. Image source: AI-generated OR scrape real photos by keyword (2-6 configurable).',
    icon: '📝',
    default_config: {
      keywords: [],
      persona: '',
  ...
  const imageText = findById('shipinhao_image_text') || FALLBACK_IMAGE_TEXT;

  const FALLBACK_REPLY_FANS: Scenario = {
    id: 'shipinhao_reply_fans_comment',
    version: '1.0.0',
    platform: 'shipinhao' as any,
    workflow_type: 'shipinhao_reply_fans_comment' as any,
    category: 'engagement',
    name_zh: '视频号 · 自动回复粉丝',
    name_en: 'WeChat Channels Reply Fan Comments',
    description_zh: '在视频号助手「评论管理」逐条回复粉丝评论。AI 按评论内容写回应，可选引流尾巴。已回复过的、自己留的评论自动跳过，只回粉丝、绝不评论作品本身。',
    description_en: 'Auto-reply to fan comments in WeChat Channels Assistant comment management. AI-tailored, optional funnel tail. Skips already-replied / your own.',
    icon: '💬',
    default_config: {
      funnel_phrase: '',
  ...
  const replyFans = findById('shipinhao_reply_fans_comment') || FALLBACK_REPLY_FANS;
  ...
  const handleConfigure = useCallback(async (scenario: Scenario | null) => {
    ...
    // 先弹登录检查 modal,确认浏览器 + 视频号助手标签 + 登录都通过再进 wizard
  ...
  const handleLoginConfirmed = () => {
    ...
    if (reason === 'shipinhao_image_text') {
      onConfigure(imageText);
    } else if (reason === 'shipinhao_reply_fans_comment') {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <section className="mb-6">
        <div className="flex flex-wrap gap-2 justify-center">
          {[
            ...
            <span
              key={i}
              className="inline-flex items-center gap-1 text-[11px] px-2.5 py-1 rounded-full border border-green-500/20 bg-green-500/5 text-gray-700 dark:text-gray-300"
            >
      ...
      {/* Login modal — 视频号没有独立创作者子域,channels.weixin.qq.com 本身就是创作后台,
          ...
          <div className="w-full max-w-sm rounded-2xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 shadow-2xl overflow-hidden"
            ...
            <div className="px-6 pt-6 pb-2 text-center">
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">
                {isZh
                  ? `视频号已经有 ${tasks.length} 个任务了，最多支持 ${MAX_TASKS} 个`
                  : `You already have ${tasks.length} WeChat Channels tasks (max ${MAX_TASKS}).`}
                <br />
                {isZh
            ...
            <div className="px-6 py-4 flex gap-2">
              ...
              <button
                ...
                className="flex-1 text-sm font-medium px-4 py-2 rounded-lg bg-gradient-to-r from-green-500 to-emerald-500 text-white hover:opacity-90 transition-opacity shadow-sm">
...
// 卡片底部操作行:左「开始」按钮(占 ~70%)+ 右「查看已有任务 »」文字入口(占 ~30%),
// 点链接跳到当前平台的「我的涨粉任务」tab。各卡片复用,避免逐卡重复样式。
...
// ── 视频号图文创作 card —— fuchsia 主色,跟抖音图文卡同源。
const ShipinhaoImageTextCard: React.FC<CardProps> = ({ loading, scenario: _scenario, onConfigure, onGoToMyTasks, isZh }) => {
  return (
    <div className="relative rounded-2xl border border-fuchsia-500/30 bg-gradient-to-br from-fuchsia-500/10 via-pink-500/5 to-transparent p-5 overflow-hidden flex flex-col">
      ...
      <div className="relative flex flex-col flex-1">
        ...
        <h3 className="text-base font-bold dark:text-white mb-1.5">
          📝 {isZh ? '视频号 · 图文创作' : 'WeChat Channels Image-Text'}
        </h3>
        <p className="text-xs text-gray-600 dark:text-gray-300 leading-relaxed mb-3 flex-1">
          {isZh
            ? '你填 3 段灵感来源(经历 / 想法 / 笔记都行),每次运行 AI 随机抽一段,按你的人设改写成视频号图文动态,配一张封面图 + 一张内容图,自动发布到视频号(也可选存草稿/仅本地)。'
...
// ── 视频号自动回复粉丝 card —— cyan 主色区分于图文(fuchsia)。
const ShipinhaoReplyFansCard: React.FC<CardProps> = ({ loading, scenario: _scenario, onConfigure, onGoToMyTasks, isZh }) => {
  return (
    <div className="relative rounded-2xl border border-cyan-500/30 bg-gradient-to-br from-cyan-500/10 via-sky-500/5 to-transparent p-5 overflow-hidden flex flex-col">
      ...
      <div className="relative flex flex-col flex-1">
        ...
        <h3 className="text-base font-bold dark:text-white mb-1.5">
          💬 {isZh ? '视频号 · 自动回复粉丝' : 'WeChat Channels Reply Fans'}
        </h3>
        <p className="text-xs text-gray-600 dark:text-gray-300 leading-relaxed mb-3 flex-1">
          {isZh
            ? '在视频号助手「评论管理」逐条回复粉丝评论,AI 按评论内容写回应,可选按概率加引流尾巴。已回复过的、自己留的自动跳过,只回粉丝、绝不评论作品本身,真人节奏间隔。'
            : 'Replies to fan comments in WeChat Channels Assistant comment management. AI-tailored replies with optional funnel tail. Skips already-replied / your own; only replies to fans, never the post itself.'}
        </p>
        <CardActionRow
          loading={loading}
          onConfigure={onConfigure}
