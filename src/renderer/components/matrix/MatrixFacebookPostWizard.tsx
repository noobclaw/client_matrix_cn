...
// 数据源清单(复用模板速生 TEMPLATE_HOTLISTS 的源名;hot 模式的 source 必须与后端 /api/web3/hot-search 的 source 名一致)。
...
const MatrixFacebookPostWizard: React.FC<Props> = ({ platformLabel, platform, accounts, accountsLoading, initialTask, onCancel, onSave }) => {
  ...
  const initSourceId = (() => {
    if (fp.sourceKind === 'news') return 'web3';
    if (fp.sourceKind === 'category') return SOURCE_OPTIONS.find((s) => s.kind === 'category' && s.catKey === fp.catKey)?.id || 'tech';
    if (fp.sourceKind === 'hot') return SOURCE_OPTIONS.find((s) => s.kind === 'hot' && s.source === fp.source)?.id || 'weibo';
    return 'web3';
  })();
