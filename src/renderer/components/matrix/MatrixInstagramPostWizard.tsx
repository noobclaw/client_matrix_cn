 ...
 *
 *   Step 1 — 勾选 N 个账号(多选)
 *   Step 2 — 数据源 + 写作语言 + 发布方式(Instagram 帖必带图,不提供「纯文字」)
 *   Step 3 — 运行频率 + 摘要 + 条款
 *
 * 与 facebook_post 的差异:Instagram 网页帖【必须带图】→ Step 2 去掉「纯文字/配图」选择,恒配图(源图优先,无则 AI 生图)。
 * 文案内联双语(不新增 i18n key)。
 */
    ...
    <div className="w-full max-w-2xl mx-auto rounded-2xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 shadow-sm overflow-hidden flex flex-col">
      <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-800 shrink-0">
        ...
        <div className="flex items-center gap-3">
          <span className="text-xs px-2.5 py-1 rounded-full border border-pink-500/40 text-pink-500 bg-pink-500/5">{T(`第 ${step} / 3 步`, `Step ${step} / 3`)}</span>
        ...
        {step === 1 && (
          <>
            <div className="rounded-lg border px-3 py-2 text-[11px] leading-relaxed border-pink-500/30 bg-pink-500/5 text-pink-700 dark:text-pink-300">
              📷 {T('勾选已登录的 Instagram 账号,每号按自己的人设从所选数据源取材,AI 原创一条图文并发布(Instagram 帖必带图,须挂 VPN)。', 'Pick logged-in Instagram accounts; each posts one AI-original post with an image from the chosen data source (Instagram posts require an image, VPN required).')}
            </div>
            <div>
