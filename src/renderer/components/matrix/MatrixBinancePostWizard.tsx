...
const PLATFORM_NAME: Record<string, string> = { douyin: '抖音', xhs: '小红书', bilibili: 'B站', kuaishou: '快手', tiktok: 'TikTok', x: 'X', binance: '币安广场', youtube: 'YouTube', shipinhao: '视频号', toutiao: '头条', gate: 'Gate广场', bitget: 'Bitget Insight', bybit: 'Bybit Byx', okx: 'OKX星球' };
...
const MatrixBinancePostWizard: React.FC<Props> = ({ platformLabel, platform, accounts, accountsLoading, initialTask, onCancel, onSave }) => {
  ...
  const [imageMode, setImageMode] = useState<'auto' | 'local'>((bp as any).imageMode === 'local' ? 'local' : 'auto');
  ...
  // 这个向导原本只服务币安,后来 gate / okx / bitget / bybit 四家广场共用它,于是标题写着
  ...
  const handleSave = async () => {
    ...
    try {
      await onSave({
        name: initialTask?.name || tp('wzBnPostDefaultTaskName').replace('{n}', String(selectedIds.length)),
  ...
  return (
    <div className="w-full max-w-2xl mx-auto rounded-2xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 shadow-sm overflow-hidden flex flex-col">
      <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-800 shrink-0">
        <div className="text-base font-semibold dark:text-white">📊 {editing ? tp('wzBnPostTitleEdit') : tp('wzBnPostTitleCreate')}</div>
          ...
          <>
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                <button type="button" onClick={() => { setImageMode('auto'); setWithImage(false); }} className={`px-3 py-2.5 rounded-lg text-sm border text-left transition-colors ${imageMode === 'auto' && !withImage ? 'border-amber-500 bg-amber-500/10 text-amber-600 dark:text-amber-400 font-medium' : 'border-gray-300 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:border-amber-500/50'}`}>
                ...
                <button type="button" onClick={() => { setImageMode('auto'); setWithImage(true); }} className={`px-3 py-2.5 rounded-lg text-sm border text-left transition-colors ${imageMode === 'auto' && withImage ? 'border-amber-500 bg-amber-500/10 text-amber-600 dark:text-amber-400 font-medium' : 'border-gray-300 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:border-amber-500/50'}`}>
                ...
                <button type="button" onClick={() => { setImageMode('local'); setWithImage(true); }} className={`px-3 py-2.5 rounded-lg text-sm border text-left transition-colors ${imageMode === 'local' ? 'border-amber-500 bg-amber-500/10 text-amber-600 dark:text-amber-400 font-medium' : 'border-gray-300 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:border-amber-500/50'}`}>
              ...
              {imageMode === 'local' && (
                <MatrixLocalImagePicker value={localImages} onChange={setLocalImages} disabled={saving} max={6} accent="amber" />
              )}
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <button type="button" onClick={() => setAutoPublish(true)} className={`px-3 py-2.5 rounded-lg text-sm border text-left transition-colors ${autoPublish ? 'border-amber-500 bg-amber-500/10 text-amber-600 dark:text-amber-400 font-medium' : 'border-gray-300 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:border-amber-500/50'}`}>
                  {i18nService.t('wzBnPostPublishAuto')}<div className="text-[11px] text-gray-400 font-normal mt-0.5">{tp('wzBnPostPublishAutoDesc')}</div>
                </button>
        ...
        {step === 4 && (
          <>
            ...
            <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 px-4 py-3 text-sm space-y-1.5">
              ...
              <SummaryRow label={i18nService.t('wzBnPostSummaryPublish')} value={autoPublish ? tp('wzBnPostSummaryPublishAuto') : i18nService.t('wzBnPostSummaryPublishDraft')} />
